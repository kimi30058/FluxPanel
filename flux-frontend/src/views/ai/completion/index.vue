<template>
  <div class="app-container completion-container">
    <el-row :gutter="20" class="completion-layout">
      <!-- 左侧输入区域 -->
          ...
          <template v-if="showUserInputForm">
            <div class="user-input-form">
              ...
              <el-form :model="userInputs" label-position="top">
                ...
                <el-form-item>
                  <el-button type="primary" @click="startCompletionByInputs">开始生成</el-button>
                </el-form-item>
              </el-form>
            </div>
          </template>
      ...
      <el-col :span="12" :xs="24" class="output-col">
        <el-card class="output-card">
          <template #header>
            <div class="card-header">
              <span>生成内容</span>
              <div class="header-actions" v-if="completionContent">
                <el-button
                  type="primary"
                  plain
                  size="small"
          ...
          <div class="completion-output">
            ...
            <div v-else-if="!completionContent" class="empty-output">
              <el-empty description="暂无生成内容" />
            </div>
            <div v-else class="output-content" v-html="formatContent(completionContent)"></div>
          </div>
        </el-card>
      </el-col>
    </el-row>
...
<script setup name="AiCompletion">
...
const completionContent = computed(() => aiStore.completionContent);
...
const handleGenerate = () => {
  ...
  aiStore.generateCompletion(inputMessage.value);
  // 不清空输入，方便用户修改后重新生成
};

// 处理清除
const handleClear = () => {
  aiStore.clearCompletionContent();
};
...
// 复制内容
const copyContent = () => {
  if (!completionContent.value) return;
...
const downloadContent = () => {
  ...
  const blob = new Blob([completionContent.value], { type: 'text/plain' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `${appInfo.value?.name || 'ai-content'}-${new Date().toISOString().slice(0, 10)}.txt`;
